...
public class Appointment_Details_UI {
	...
	private String[] formatted_appointments = Algorithms.gather_appointments();

	private String get_record_id(String time, String date, String brief, String feedback, String client_id,
			String employee_id, String location_id) {
		List<Appointment> appointments = Main.appointments_array;
		for (Appointment appointment : appointments) {
			if (time.equals(appointment.time) && date.equals(appointment.date) && brief.equals(appointment.brief)
					&& feedback.equals(appointment.feedback) && client_id.equals(appointment.client_id)
					&& employee_id.equals(appointment.employee_id) && location_id.equals(appointment.location_id)) {
	...
	private static String[] extract_client_details(String formatted_string) {
		...
		while(counter < formatted_string.length()) {
			if(formatted_string.charAt(counter) != ' ' && formatted_string.charAt(counter) != '-') {
				output[array_pos] += String.valueOf(formatted_string.charAt(counter));
			} else if(formatted_string.charAt(counter) == ' ' && spaces_counter < 2) {
				array_pos++;
				spaces_counter++;
			}
			counter++;
	...
	private static String[] extract_location_details(String formatted_string) {
		String[] output = new String[3];
		output[0] = "";
		output[1] = "";
		output[2] = "";
		...
		while(counter < formatted_string.length()) {
			if(formatted_string.charAt(counter) != ' ' && formatted_string.charAt(counter) != ',') {
				output[array_pos] += String.valueOf(formatted_string.charAt(counter));
			} else if(formatted_string.charAt(counter) == ',' && comma_counter < 2) {
				array_pos++;
				comma_counter++;
			}
			counter++;
	...
	private void reset_listview(List<Appointment> appoinment, ObservableList<String> observable_list, ListView<String> listview) throws SQLException {
	...
	private String extract_employee_id(String formatted_employee) {
		String output = null;
		for(int i = 0; i < formatted_employee.length(); i++) {
			if(String.valueOf(formatted_employee.charAt(i) != ' ') != null) {
				if(output == null) {
					output = String.valueOf(formatted_employee.charAt(i));
				} else {
					output += String.valueOf(formatted_employee.charAt(i));
				}
			} else {
		...
		if (administrator) {
			...
			btn_navigate_home.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					...
					System.out.println(
							"Navigate from Appointment_Details_Administrator_UI to Home_Screen_Administrator_UI.");
			...
			btn_navigate_clients.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					UI_Templates.is_appt_details_shown = false;
					System.out.println(
							"Navigate from Appointment_Details_Administrator_UI to Clients_Screen_UI (Administrator).");
					...
					client_screen_screen.getStylesheets()
			...
			btn_navigate_employees.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					UI_Templates.is_appt_details_shown = false;
					System.out.println(
							"Navigate from Appointment_Details_Administrator_UI to Employee_Details_Screen_Administrator_UI.");
					...
					employee_details_screen.getStylesheets()
							.add(getClass().getResource("application.css").toExternalForm());
			...
			btn_navigate_trades.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					UI_Templates.is_appt_details_shown = false;
					System.out.println(
							"Navigate from Appointment_Details_Administrator_UI to Trade_List_Screen_Administrator_UI.");
			...
			navigation_button_bar.getChildren().addAll(btn_navigate_appointments, btn_navigate_home,
					btn_navigate_clients, btn_navigate_employees, btn_navigate_trades);
		} else {
			btn_navigate_home.setOnAction(new EventHandler<ActionEvent>() {
				@Override
				public void handle(ActionEvent e) {
			...
			btn_navigate_appointments.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					...
					appointment_list_screen.getStylesheets()
							.add(getClass().getResource("application.css").toExternalForm());
	...
	private Popup calendar_popup(Stage primary_stage) {
		...
		Pane calendar = UI_Templates.Calender(primary_stage, true, Algorithms.dimension_calculator(600.0, false),
				Algorithms.dimension_calculator(400.0, true));
	...
	private GridPane centre_panel(Stage primary_stage, boolean administrator, String date, Appointment appointment) {
		...
		ta_customer_feedback.setMinSize(Algorithms.dimension_calculator(200.0, false),
				Algorithms.dimension_calculator(150.0, true));
		if (!administrator) {
			ta_customer_feedback.setEditable(false);
			Button btn_send_feedback = new Button("Send");
			btn_send_feedback.setMinSize(Algorithms.dimension_calculator(40.0, false),
			...
			btn_send_feedback.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					...
					Update_DB.Update_String_Record(Main.url, Main.user, Main.password, Main.user_data.business,
							"appointments", "feedback", null, ta_customer_feedback.getText());
				}
			});
		...
		ta_brief.setMinSize(Algorithms.dimension_calculator(200.0, false),
				Algorithms.dimension_calculator(150.0, true));
		...
		for(int i = 0; i < Main.appointments_array.size(); i++) {
			masterview_items.add(formatted_appointments[i]);
		}
		...
		Algorithms.item_filter_listener(tf_search,formatted_appointments,masterview_items,lv_masterview);
		left_view.getChildren().addAll(lbl_masterview_titles, hb_search, lv_masterview);

		if (administrator) {
			TextField tf_client = new TextField();
			centre_view.add(tf_client, 1, 2);
			TextFields.bindAutoCompletion(tf_client, formatted_clients);
			...
			TextFields.bindAutoCompletion(tf_employee, formatted_employees);
			...
			if(appointment != null) {
				String formatted_location = Algorithms.location_details(appointment.location_id);
				String[] extracted_location_info = extract_location_details(formatted_location);
				lbl_date.setText(appointment.date);
				lbl_time.setText(appointment.time);
				tf_client.setText(Algorithms.client_details_output(appointment.client_id));
				tf_street_number.setText(extracted_location_info[0]);
				tf_street_name.setText(extracted_location_info[1]);
				tf_postcode.setText(extracted_location_info[2]);
				tf_employee.setText(Algorithms.employee_details_output(appointment.employee_id));
				ta_brief.setText(appointment.brief);
				ta_customer_feedback.setText(appointment.feedback);
			}
			...
			btn_add.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					...
					if (!time_valid) {
						System.out.println("Logical Error: time allowing strings over or below 5 characters.");
					}
					// String(8-10) date
					String date_s = lbl_date.getText();
					boolean date_valid = date_s.length() > 7 && date_s.length() < 11 ? true : false;
					if (!date_valid) {
						System.out.println("Logical Error: date allowing strings under 8 or above 10 characters.");
					}
					// String(<1000) brief
					String brief_s = ta_brief.getText();
					boolean brief_valid = brief_s.length() < 1000 ? true : false;
					if (!brief_valid) {
						System.out.println("Logical Error: brief allowing strings over 999 characters.");
					}
					...
					if (!feedback_valid) {
						System.out.println("Logical Error: customer_feedback allowing strings over 999 characters.");
					}
					// String(7) client_id
					String[] client_details = extract_client_details(tf_client.getText());
					String client_id_s = Algorithms.output_client_id(client_details[0], client_details[1], client_details[2]);
					boolean client_exists = Algorithms.client_exists_id(client_id_s);
					// String(7) employee_id
					String employee_id_s = extract_employee_id(tf_employee.getText());
					...
					if (!street_number_valid) {
						System.out.println("Logical Error: street_number allowing number strings over 7 characters.");
					}
					...
					if (!street_name_valid) {
						System.out.println("Logical Error: street_name allowing strings over 50 characters.");
					}
					// Int(=4) postcode
					int postcode_int = Integer.parseInt(tf_postcode.getText());
					boolean postcode_valid = postcode_int < 10000 ? true : false;
					if (!postcode_valid) {
						System.out.println("Logical Error: postcode allowing integers over 4 digits.");
					}
					...
					if (!location_exists && location_valid) {
						...
						try {
							Main.locations_array = Select_DB.Extract_Data_Record_Locations(Main.url, Main.user,
									Main.password, Main.user_data.business);
					...
					if (time_valid && date_valid & brief_valid && feedback_valid && client_exists && employee_exists && location_valid) {
						try {
							if (Algorithms.appointment_exists(time_s, date_s, brief_s, feedback_s, client_id_s,employee_id_s, location_id_s)) {
								...
								Main.appointments_array = Select_DB.Extract_Data_Record_Appointments(Main.url, Main.user, Main.password, Main.user_data.business);
								formatted_appointments = Algorithms.gather_appointments();
							}
							List<Appointment> array = Main.appointments_array;
							int max_pos = Main.appointments_array.size() - 1;
							String item_add_string = String.format("%s, %s\nClient: %s\nEmployee: %s\nLocation: %s",
									array.get(max_pos).time,
									array.get(max_pos).date, 
									Algorithms.client_details_output(array.get(max_pos).client_id),
									Algorithms.employee_details_output(array.get(max_pos).employee_id),
									Algorithms.location_details(array.get(max_pos).location_id));
							masterview_items.add(item_add_string);
							lv_masterview.setItems(masterview_items);
						} catch (SQLException e1) {
							e1.printStackTrace();
						}
			...
			btn_update.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					if (Main.appointments_array != null) {
						if (lbl_date.getText() == null || lbl_time.getText() == null || tf_client.getText() == null
								|| tf_street_number.getText() == null || tf_street_name.getText() == null
								|| tf_postcode.getText() == null || tf_employee.getText() == null
								|| ta_brief.getText() == null) {
							UI_Templates.error_popup(primary_stage, "Cannot update information when fields are blank.");
						} else {
							String location_id_s = Algorithms.output_location_id(tf_street_number.getText(),tf_street_name.getText(), Integer.parseInt(tf_postcode.getText()),tf_client.getText());
							String record_id = get_record_id(lbl_time.getText(),lbl_date.getText(),ta_brief.getText(),ta_customer_feedback.getText(),tf_client.getText(),tf_employee.getText(),location_id_s);
							if (record_id != null) {
								try {
									...
									Update_DB.Update_String_Record(Main.url,Main.user,Main.password,Main.user_data.business,"appointments","client_id",record_id,tf_client.getText());
									Update_DB.Update_String_Record(Main.url,Main.user,Main.password,Main.user_data.business,"appointments","employee_id",record_id,tf_employee.getText());
									...
									reset_listview(Main.appointments_array, masterview_items, lv_masterview);
									Main.appointments_array = Select_DB.Extract_Data_Record_Appointments(Main.url, Main.user, Main.password, Main.user_data.business);
									formatted_appointments = Algorithms.gather_appointments();
	...
	public BorderPane get_scene(Stage primary_stage, boolean administrator, String date, Appointment appointment_details) {
		BorderPane border_pane = new BorderPane();
		border_pane.setTop(header(primary_stage, administrator));
		border_pane.setCenter(centre_panel(primary_stage, administrator, date, appointment_details));
		return border_pane;
	}
}
